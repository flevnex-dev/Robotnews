...
class db_class {
    ...
    public function open() {
        $con = mysqli_connect("localhost", "root", "", "robotnewsbd");
        mysqli_set_charset($con,"utf8");
        return $con;
    }

    public function pdocon() {
        $db = new PDO("mysql:host=localhost;dbname=robotnewsbd;", "root", "");
    ...
    function baseUrl($suffix = '') {
        ...
        $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "project/robotnews/cms-admin/";
        $suffix = ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }

    function baseUrlF($suffix = '') {
        $protocol = strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "project/robotnews/";
        $suffix = ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
...
//        $con=$this->open();
//        $st=mysqli_real_escape_string($con,$string);
//        $this->close($con);
//        
//        return $st;
    ...
    public function CleanParam($string) {
        $con=$this->open();
