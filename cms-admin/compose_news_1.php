...
if (isset($_POST['create'])) {
    extract($_POST);
    if (!empty($news_headding) && !empty($news_sub_headding) && !empty($reporter) && !empty($_FILES['news_thumble']['name']) && !empty($news_short_details) && !empty($news_long_details) && !empty($home_page) && !empty($breaking_news) && !empty($tab_lead_news) && !empty($_FILES['slider_image']['name']) && !empty($news_publish) && !empty($news_status) && !empty($news_date_time) && !empty($news_robot)) {
        ...
        $insert = array('news_headding' => $news_headding,
            ...
            'slider_image' => $slider_image, 'news_publish' => $news_publish, 'news_status' => $news_status, 'news_date_time' => $news_date_time, 'news_robot' => $news_robot, 'date' => date('Y-m-d'), 'status' => 1);
    ...
    if (!empty($news_headding) && !empty($news_sub_headding) && !empty($reporter) && !empty($news_short_details) && !empty($news_long_details) && !empty($home_page) && !empty($breaking_news) && !empty($tab_lead_news) && !empty($news_publish) && !empty($news_status) && !empty($news_date_time) && !empty($news_robot)) {
        ...
        if (!empty($_FILES['news_thumble']['name'])) {
            $news_thumble_1 = $imgclassget->upload_fiximage("upload", "news_thumble", "news_thumble_upload_" . $table_name . "_" . time());
        ...
        $upd2 = array('news_headding' => $news_headding, 'news_sub_headding' => $news_sub_headding, 'reporter' => $reporter, 'news_thumble' => $news_thumble, 'news_short_details' => $news_short_details, 'news_long_details' => $news_long_details, 'home_page' => $home_page, 'breaking_news' => $breaking_news, 'tab_lead_news' => $tab_lead_news, 'slider_image' => $slider_image, 'news_publish' => $news_publish, 'news_status' => $news_status, 'news_date_time' => $news_date_time, 'news_robot' => $news_robot, 'date' => date('Y-m-d'), 'status' => 1);
                    ...
                    if (isset($_GET['edit'])) {
                        ...
                        <div class="widget-body"><form enctype='multipart/form-data' class="form-horizontal" method="post" action="" role="form">
                                <input type="hidden" name="id" value="<?php echo $_GET['edit']; ?>"><div class='form-group'>
                                    ...
                                    <div class='col-sm-9'>
                                        <input type='text' id='form-field-1' name='news_date_time' value='<?php echo $obj->SelectAllByVal($table, "id", $_GET['edit'], "news_date_time"); ?>' placeholder='News Date Time' class='form-control' />
                                    </div>
                                </div><div class='form-group'>
